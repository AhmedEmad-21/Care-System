...
بدل الاعتماد على ملف JSON داخل المشروع، استخدم المتغيرات التالية في ملف `.env` أو في منصة الاستضافة:
...
## 1️⃣ أولاً: إعداد Firebase في Node.js عبر Environment Variables

1. أضف القيم السابقة إلى ملف `.env` أو إلى إعدادات البيئة في الاستضافة.

   > ⚠️ **ملاحظة أمنية**: لا تضع ملف JSON السري داخل المشروع.
...
_لتخزين توكن الـ FCM الخاص بجهاز كل مستخدم:_
...
_لتخزين الإشعارات لعرضها داخل صفحة التنبيهات في التطبيق:_
...
- **Method**: `PATCH`
- **URL**: `/api/notifications/:id/read` _(مثال: `/api/notifications/notif_101/read`)_
- **Headers**: `Authorization: Bearer <user_token>`
- **Response** (`200 OK`):
  ```json
  {
...
- **Method**: `DELETE`
- **URL**: `/api/notifications/:id` _(مثال: `/api/notifications/notif_101`)_
- **Headers**: `Authorization: Bearer <user_token>`
- **Response** (`200 OK`):
  ```json
  {
...
async function sendNotificationToUser({
  ...
  try {
    ...
    const userTokens = await UserDeviceTokens.findAll({
